...
describe('lib/kinesis-client', () => {
  ...
  `('$methodName', ({ isRetriable, methodName }) => {
    ...
    test.skip(`${methodName} throws exceptions from the wrapped SDK call`, async () => {
    ...
    test.skip(`${methodName} throws exceptions from the wrapped SDK promise`, async () => {
      error = Object.assign(new Error('foo'), { code: 'MissingRequiredParameter' });
      sdkClient[methodName].mockImplementationOnce(() => ({
    ...
    test.skip(`${methodName} throws exceptions with a debuggable stack trace`, async () => {
      error = Object.assign(new Error('foo'), { code: 'MissingRequiredParameter' });
      sdkClient[methodName].mockImplementationOnce(() => ({
        promise: () => Promise.reject(error)
      }));
    ...
    if (methodName === 'describeStreamSummary') {
      ...
      test.skip('describeStreamSummary should throw standard errors', async () => {
        error = new Error('foo');
        sdkClient.describeStreamSummary.mockImplementationOnce(() => {
          return {
            promise: () => {
