...
function getStats(streamName) {
  ...
  const formattedStats = Object.entries(moduleStats).reduce(
    (accumulator, element) => formatValues(accumulator, element),
    { version }
  );
  ...
  if (streamName) {
    ...
    baseStats.kinesis = Object.entries(streamData).reduce(
      (accumulator, element) => formatValues(accumulator, element),
      {}
    );
    return baseStats;
  ...
  const formattedStreams = Object.entries(streams).reduce((results, [key, value]) => {
    const finalValue = Object.entries(value).reduce(
      (accumulator, element) => formatValues(accumulator, element),
      {}
    );
